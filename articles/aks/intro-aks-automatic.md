...
ms.date: 04/13/2026
author: sabbour, wangyira
ms.author: asabbour, wangyira
...
| Option | AKS Automatic | AKS Standard |
|--------|---------------|--------------|
| Node management | **Preconfigured**: AKS Automatic manages the node pools using [Node Autoprovisioning][node-autoprovisioning]. **Optional**: [AKS Automatic with managed system node pools][managed-system-node-pool] creates, scales, upgrades the system nodes and system components on your behalf and host them in the AKS subscriptions. | **Default**: You create and manage system and user node pools <br> **Optional**: AKS Standard manages user node pools using [Node Autoprovisioning][node-autoprovisioning]. |
...
| Node operating system | **Preconfigured for system node pool**: [Azure Linux][azure-linux] **Optional for user nodes**: Azure Linux, | **Default**: Ubuntu <br> **Optional**: <br> * [Azure Linux][azure-linux] <br> * [Windows Server][windows-server] |
...
| Cluster upgrades | **Preconfigured**: Clusters are [automatically upgraded using the stable channel to minor version N-1, where N is the latest supported minor version][cluster-upgrades]. |  **Default**: Manual upgrade. <br> **Optional**: Automatic upgrade using a selectable [upgrade channel][cluster-upgrade-channels].  |
| Node OS image upgrades | **Preconfigured** Clusters are [automatically upgraded using the NodeImage channel with security fixes and bug fixes][nodeos-updates] | **Default**: Manual upgrade. <br> **Optional**: Automatic upgrade using a selectable [node OS upgrade channel][nodeos-updates]|
